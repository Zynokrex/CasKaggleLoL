...
def preProcessing(df,return_full=True,finished_games=True):
    ...
    '''
    Returns the dataset after creating the needed variables.
    
        Parameters:
            df(pandas.core.frame.DataFrame): DataFrame containing lol_ranked_games.csv
            return_full(bool): True if you want to return a complete dataframe with new features
            finished_games(bool): True if you want to return the finished games data
        
        Returns:
            dinishedDfs (list): List containing the processed dataframe         
    '''
    finishedDfs=[]
